...
(cat BitSet.hpp | grep "vector<bool>" > /dev/null) && { FAILED=1; echo "FAILED: not allowed to use vector<bool>!"; };

FAILED=0
clang-format  --dry-run --Werror *.cpp *.hpp  || { FAILED=1; echo 'FAILED: code formatting. Run `clang-format -i *.cpp`.'; }
...
clang-tidy -p build/compile-commands.json --config-file="../.clang-tidy" *.cpp *.hpp || { FAILED=1; echo 'FAILED: clang-tidy failed'; }
